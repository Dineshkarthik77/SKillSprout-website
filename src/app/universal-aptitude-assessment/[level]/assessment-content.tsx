...
export function UniversalAptitudeAssessmentContent({ level }: { level: number }) {
  ...
  const fetchQuiz = useCallback(async () => {
    ...
    try {
      const result = await generateQuiz({ domain: 'Universal Aptitude', level });
      setQuizData(result);
      setPageState('loaded');
    } catch (error) {
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center h-96">
        ...
        <h2 className="text-2xl font-headline font-semibold mt-8">
          Generating Your Universal Aptitude Assessment...
        </h2>
