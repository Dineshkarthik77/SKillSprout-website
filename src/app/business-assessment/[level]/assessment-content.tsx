...
export function BusinessAssessmentContent({ level }: { level: number }) {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      ...
      try {
        const result = await generateQuiz({ domain: 'Business & Finance', level });
        setQuizData(result);
        setPageState('loaded');
      } catch (error) {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex < quizData.questions.length - 1) {
      ...
      // End of quiz logic
      if (level < 3) {
        router.push(`/business-assessment/${level + 1}`);
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center h-96">
        ...
        <h2 className="text-2xl font-headline font-semibold">
          Generating Your Business & Finance Assessment...
        </h2>
