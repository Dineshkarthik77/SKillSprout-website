...
export function LegalAssessmentContent({ level }: { level: number }) {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      ...
      try {
        const result = await generateQuiz({ domain: 'Legal', level });
        setQuizData(result);
        setPageState('loaded');
      } catch (error) {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex < quizData.questions.length - 1) {
      ...
      // End of quiz logic
      if (level < 3) {
        router.push(`/legal-assessment/${level + 1}`);
      } else {
        router.push(`/career-storyboard?domain=Legal`);
      }
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center h-96">
        ...
        <h2 className="text-2xl font-headline font-semibold">
          Generating Your Legal Assessment...
        </h2>
